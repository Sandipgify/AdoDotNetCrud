...
{
    public class DbAccessManager : IDbAccessManager
    {
        private readonly string connString = string.Empty;
        public DbAccessManager(IConfiguration configuration)
        {
        ...
        private void AddParameters(object parameters, SqlCommand command)
        {
            if (parameters != null)
            {
                ...
                {
                    command.Parameters.AddWithValue($"@{p.Name}", p.GetValue(parameters)??DBNull.Value);
                }
            }
        }
